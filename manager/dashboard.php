...
// Pending letters (waiting for manager approval)
$stmt = $db->query("SELECT COUNT(*) as count FROM letters WHERE status = 'pending'");
$stats['pending_letters'] = $stmt->fetch()['count'];

// Letters approved by manager (waiting for director)
$stmt = $db->prepare("SELECT COUNT(*) as count FROM letters WHERE status = 'manager_approved' AND manager_action_by = ?");
$stmt->execute([$_SESSION['user_id']]);
$stats['approved_by_me'] = $stmt->fetch()['count'];

// Total letters processed by this manager
$stmt = $db->prepare("SELECT COUNT(*) as count FROM letters WHERE manager_action_by = ?");
$stmt->execute([$_SESSION['user_id']]);
$stats['total_processed'] = $stmt->fetch()['count'];

// Letters rejected by manager
$stmt = $db->prepare("SELECT COUNT(*) as count FROM letters WHERE status = 'rejected' AND manager_action_by = ?");
$stmt->execute([$_SESSION['user_id']]);
$stats['rejected_by_me'] = $stmt->fetch()['count'];

// Recent pending letters
$stmt = $db->query("
...
<head>
    ...
    <title>Dashboard Manager - <?php echo APP_NAME; ?></title>
    ...
    <main class="container">
        <div class="row">
            <div class="col-12">
                ...
                <p>Selamat datang, <?php echo htmlspecialchars($_SESSION['full_name']); ?>. Berikut adalah ringkasan surat yang perlu direview.</p>
            </div>
        </div>
            ...
            <div class="col-8">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Surat Menunggu Review</h3>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <?php
                    // Get recent letters processed by this manager
                    $stmt = $db->prepare("
                        SELECT l.*, t.name as template_name 
                        FROM letters l 
                        LEFT JOIN templates t ON l.template_id = t.id 
                        WHERE l.manager_action_by = ?
                        ORDER BY l.manager_action_at DESC 
                        LIMIT 5
                    ");
                    $stmt->execute([$_SESSION['user_id']]);
                            ...
                            <tbody>
                                <?php foreach ($recentActivity as $activity): ?>
                                    <tr>
                                        ...
                                        <td><?php echo formatDate($activity['manager_action_at'], 'd M Y H:i'); ?></td>
                                        <td><?php echo htmlspecialchars($activity['manager_notes'] ?: '-'); ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
